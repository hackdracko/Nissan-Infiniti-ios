...
@interface SEECTypeViewController ()

@end

@implementation SEECTypeViewController
...
- (void)viewDidLoad
{
    ...
    self.items = [[NSArray alloc] initWithObjects:@"Evaluaciones",@"Indices de Satisfacción", nil];
    [super viewDidLoad];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    NSLog(@"numero de elementos %d", items.count);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@" value of  %@", [items objectAtIndex:indexPath.row]);
    
    
    NSLog(@"what is the index  = %d", indexPath.row);
    NSLog(@"what you want   = %@", [items objectAtIndex:indexPath.row]);
   ...
   NSArray *objts = [NSArray arrayWithObjects:[items objectAtIndex:indexPath.row], [NSString stringWithFormat:@"%d", indexPath.row], nil];
    
    
   NSDictionary *typesReplica = [NSDictionary dictionaryWithObjects:objts  forKeys:objKeys];
    
    
    
   [[NSNotificationCenter defaultCenter] postNotificationName:@"typeSelected" object:typesReplica];
